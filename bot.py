...
def lalala(message):
	if message.chat.type == 'private':
		...
		if message.text == '⚽️ИГРАТЬ⚽️':
			...
			bot.send_message(message.chat.id, 'Хорошо, его имя Александр. Не обижай одмена) График роботы админа: с 8:00-23:00. Отвечать может с задержкой, просьба не флудить.', parse_mode='html', reply_markup=markup)
			...
			item6 = types.InlineKeyboardButton("ВЫГОДА ПРОЕКТА", callback_data='info_6')
	...
	try:
		if call.message:
			if call.data == 'play_1':
				...
				bot.send_message(call.message.chat.id, '''✅✅✅ ЦАРСТВО №1 ВЫБРАНО ✅✅✅\nПиши Админу и говори какое царство выбрал) График роботы админа: с 8:00-23:00. Отвечать может с задержкой, просьба не флудить.''', parse_mode='html', reply_markup=markup)
			...
			# 	bot.send_message(call.message.chat.id, '{} {}'.format('👑ЦАРСТВО №10👑\n', line10))
				...
				bot.send_message(call.message.chat.id, """1.Ознакомится с доступной информацией, выбрать доступное Царство, следовать инструкциям бота.
					...
					для рекламы в различных проектах)
				...
				bot.send_message(call.message.chat.id, """Никакого обмана нет, при желании все легко поддается анализу, внося 250 гривен в фонд игры (200 грн ЦАРЮ, 50 грн на пиар проекта) и 
